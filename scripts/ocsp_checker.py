...
def check_ocsp_stapled(tls_socket: Any) -> Dict[str, Any]:
    ...
    try:
        # Python 3.14+ uses get_verified_chain() DER bytes;
        # ocsp_response() is not available in this Python build,
        # so we rely on direct OCSP instead.
        result["ocsp_error"] = "stapling API not available"
    except Exception as exc:
        result["ocsp_error"] = str(exc)
...
def _get_issuer_spki(tls_socket: Any) -> Optional[bytes]:
    ...
    Uses ``get_verified_chain()`` (available in Python 3.14+) to obtain
    the full certificate chain.  The second certificate in the chain is
    the issuer (if the server sent intermediates).
    """
    try:
