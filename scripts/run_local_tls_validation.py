...
CLASSIFICATION = dict(CLASSIFICATION_TO_OVERALL_STATUS)
CLASSIFICATION_ORDER = list(ALL_CLASSIFICATIONS)
...
def _check_deprecated_tls(domain: str, ip: str, ca_store: str = "platform", timeout: float = 10.0) -> bool:
        ...
        with socket.create_connection((ip, DEFAULT_PORT), timeout=timeout) as sock:
            with ctx.wrap_socket(sock, server_hostname=domain) as tls:
                try:
                    ...
                    if chain:
                        ...
                        if chain_len <= 1:
                            ...
                            try:
                                ...
                                logger.debug("Could not parse issuer org for chain subtype: %s", e)
                    ...
                    # get_verified_chain() not available on Python < 3.14
                    subtype = "missing_intermediate"
    except Exception as e:
        logger.debug("Chain subtype determination failed for %s: %s", domain, e)
        subtype = "unknown"
    return subtype
...
def _attempt_tls_handshake(domain: str, ip: str, ca_store: str = "platform", timeout: float = 10.0) -> Dict[str, Any]:
    ...
    try:
        with socket.create_connection((ip, DEFAULT_PORT), timeout=timeout) as sock:
            with context.wrap_socket(sock, server_hostname=domain) as tls:
                ...
                try:
                    ...
                    # get_verified_chain() not available on Python < 3.14
                    # or raised an error; assume chain is complete from handshake success
                    info["cert_chain_complete"] = True
                ...
                try:
                    ...
                    logger.debug("Could not check TLS compression for %s: %s", domain, e)
        ...
        if info["error_category"] in ("UNTRUSTED_CHAIN", "INCOMPLETE_CHAIN"):
            info["chain_subtype"] = _determine_chain_subtype(domain, ip, timeout=timeout)
...
def probe_domain(domain: str, ca_store: str = "platform", timeout: float = 10.0) -> Dict[str, Any]:
    ...
    if result["classification"] == "VALID_TLS":
        try:
            ...
            try:
                ...
                tls_info["deprecated_tls_check"] = "rejected"
        except (AttributeError, ValueError):
            tls_info["deprecated_tls_detected"] = False
...
def _build_seed_report(results: List[Dict[str, Any]], elapsed: float) -> str:
    ...
    lines.extend([
        ...
        "- The existing `RealValidationRunner` was not modified.",
        "- OFE signals remain `HOLD_PENDING_REAL_DATA`.",
        "- No signal was promoted.",
        "- No threshold tuning occurred.",
        "",
        "## OFE Comparison",
        "",
        "OFE comparison requires a 5000+ row pre-collected TLS dataset with labeled samples.",
        "The local probe runner does not generate OFE-compatible labels automatically.",
        "To perform OFE comparison, collect real TLS probe results into a JSONL/CSV dataset",
        ...
        "```",
        "",
        "Currently, SPL and OFE analysis is not applied to probe results because:",
        "- Probe results do not include `label` or `label_weight` fields (required by the contract).",
        "- Probe results are observational, not ground-truth labeled.",
        "- A separate dataset collection step is needed to generate SPL-compatible input.",
        "",
...
def _build_mixed_report(
    ...
    """Build a report for mixed-classification datasets with optional accuracy metrics."""
    total = len(results)
    success = sum(1 for r in results if r.get("classification") == "VALID_TLS")
    failure = total - success
    ...
    lines = [
        "# Mixed TLS Validation — Evidence Report",
        "",
        f"Generated: {datetime.now(timezone.utc).isoformat()}Z",
        f"Runner: `scripts/run_local_tls_validation.py`",
        f"Duration: {elapsed:.1f}s",
    ...
    lines.extend(["", "### TLS Version Distribution", ""])
    for ver, count in sorted(tls_versions.items()):
        lines.append(f"- {ver}: {count} domains")
    lines.append("")
    ...
    if accuracy:
        lines.extend([
            ...
            f"| Metric | Value |",
            f"|---|---|",
            f"| Accuracy | {accuracy.get('accuracy_pct', 0)}% |",
            f"| Correct | {accuracy.get('correct', 0)} |",
            f"| Total evaluated | {accuracy.get('total_with_results', 0)} |",
            "",
        ])
        per_cat = accuracy.get("per_category", {})
        if per_cat:
            lines.extend([
            ...
            for cat, stats in sorted(per_cat.items()):
                lines.append(
                    f"| {cat} | {stats.get('expected_count', 0)} | "
                    f"{stats.get('correct', 0)} | {stats.get('accuracy_pct', 0)}% |"
                )
            lines.append("")

        misclass = accuracy.get("misclassified_examples", [])
        if misclass:
            lines.extend([
                "### Misclassified Examples",
                "",
                "| Domain | Expected | Actual | Error |",
                "|---|---|---|---|",
            ])
            for ex in misclass[:20]:
                err_msg = (ex.get("error") or "")[:80]
                lines.append(
                    f"| {ex['domain']} | {ex['expected']} | {ex['actual']} | {err_msg} |"
                )
            lines.append("")
    ...
    lines.extend([
        ...
        "4. **Single IP per domain** — only one A record is probed (first resolved).",
        "5. **No IPv6** — probes use IPv4 only.",
        f"6. **Rate limited** — {RATE_LIMIT_SECONDS}s delay between probes to avoid aggressive scanning.",
        f"7. **Timeout** — each connection has a {PROBE_TIMEOUT}s hard limit.",
        "8. **OCSP best-effort** — revocation is checked when the AIA OCSP responder is reachable.",
        "",
        "---",
        "",
        "_This report is for local evidence gathering only. No production claims are made._",
